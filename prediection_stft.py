...
def process_audio(file_path, sr=4800, n_fft=512, hop_length=64, desired_time=2.0):
    # 預處理音訊
    feature = preprocess_stft_audio(file_path, traget_sr=sr, n_fft=n_fft, hop_length=hop_length, desired_time=desired_time)
    return feature
    ...
    if not os.path.isdir(test_audio_dir):
        ...
        for file in os.listdir(test_audio_dir):
            ...
            if not file.endswith(('.wav', '.WAV')):
                ...
                continue
            try:
                feature = process_audio(file_path, sr=4800, desired_time=2.0, n_fft=512, hop_length=64)
                # 呼叫預測函數
